...
3. The workflow fetches `corpCode.xml`, searches recent major-report disclosures for buyback-related report names, calls structured decision APIs for discovered event companies, scans listed companies for `stockTotqySttus` / treasury holding snapshots, then builds the static Vite site.

The default live discovery window is the last 89 days because OpenDART `list.json` without `corp_code` is limited to roughly three months. The scheduled workflow uses disclosure-discovered companies for event enrichment (`--stock-codes ALL`) and scans all listed companies for the latest annual-report holding snapshots (`--holding-stock-codes ALL`, `--report-codes 11011`). This keeps the top holding-ratio chart based on actual treasury-share ratios across the available listed universe instead of only recent disclosure candidates. Historical backfills should widen `--report-codes` or run a separate controlled job.
